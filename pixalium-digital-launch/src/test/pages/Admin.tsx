...
import { LogOut, Package, MessageSquare, Image, BarChart3, Trash2, Eye, CheckCircle, XCircle, Plus, Send, Megaphone, X, Trophy } from "lucide-react";
...
import {
  ...
  type Order, type ContactMessage, type PortfolioProject, type Product, type DownloadCode, type Tournament,
} from "@/lib/firebase";
...
type Tab = "stats" | "orders" | "contacts" | "portfolio" | "testimonials" | "newsletter" | "promo" | "products" | "downloadCodes" | "tournaments";
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const unsub9 = subscribeToRecords<Tournament>("tournaments", setTournaments);
  ...
  // Build tabs dynamically - hide promo and tournaments for non-authenticated users
  ...
  // Only show promo tab if user is authenticated
  if (user) {
    tabs.push({ key: "promo", label: "Promotions", icon: Megaphone });
  }
  ...
  // Only show tournaments tab if user is authenticated
  if (user) {
    tabs.push({ key: "tournaments", label: "Tournois", icon: Trophy });
  }
